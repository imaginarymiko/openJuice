 ...
 * @module openjuice.engine.save:ProfileManager
...
export module openjuice.engine.save:ProfileManager;
...
BEGIN_MODULE_NAMESPACE(openjuice::engine::save);
...
public:
    ...
    public:
         ...
         *
         * The default parameters should only be used when constructing a new profile.
         */
...
private:
    ...
    Expected<void, Error> deserialize(const TomlTable& table) noexcept {
        try {
            if (NodeView<const TomlNode> playerNameNode = table["playerName"]; playerNameNode.node() != nullptr) {
                if (Optional<String> playerNameValue = playerNameNode.value<String>(); playerNameValue.has_value()) {
            ...
            if (NodeView<const TomlNode> playTimeNode = table["totalPlayTime"]; playTimeNode.node() != nullptr) {
                if (Optional<i64> playTimeValue = playTimeNode.value<i64>(); playTimeValue.has_value()) {
