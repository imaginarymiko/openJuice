...
public:
    ...
    void configure(Binder& binder) override {
        ...
        auto bindLoggersFor = [&binder, this]<typename... Ts> -> void {
            (binder.bind<SharedPointer<Logger>>()
                .annotated_with(Named(*Class<Ts>().name()))
                .to_provider([this] -> SharedPointer<Logger> { return logging->of<Ts>(); }), ...);
        };

        bindLoggersFor.template operator()<
            SettingsService,
            LocalizationService,
            ProfileManager,
            BoardLibrary,
